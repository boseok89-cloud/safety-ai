...
export default function App() {
  ...
  const fillTemplateWithAI = async () => {
    if (!companyTemplate) { alert("먼저 회사 양식을 업로드해주세요!"); return; }
    ...
    try {
      ...
      const systemPrompt = `산업안전보건 전문가입니다. 아래 위험성평가 양식 구조를 분석하고 사업장 정보로 빈칸을 채워주세요.\n\n양식:\n${templateText.slice(0, 2000)}\n\nJSON 배열로만 응답: [{"row":행번호,"col":열번호,"value":"내용"}]\nJSON 외 텍스트 금지.`;
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      rows.forEach((row, ri) => {
        row.forEach((cell, ci) => {
          ...
          if (!newWs[nc] || !newWs[nc].v) {
            ...
            else if (s.includes("근로자수")) newWs[nc] = { t: "s", v: baseInfo.workers || "" };
  ...
  const downloadAllExcel = () => {
    ...
    const summaryData = [
      ...
      ["", ""],
      ["AI 초안입니다. 안전관리자가 반드시 검토 후 사용하세요.", ""],
    ];
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "28px 16px" }}>
          ...
          <button onClick={async () => { setMode("moel"); await saveStorage("eval-mode", "moel"); }} style={{ width: "100%", background: "#fff", border: `2px solid ${C.accent}`, borderRadius: 16, padding: "20px 18px", marginBottom: 12, display: "flex", alignItems: "flex-start", gap: 14, cursor: "pointer", textAlign: "left" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: C.slate, lineHeight: 1.6 }}>법정 6단계 절차에 따라 AI가 문서를 자동 작성해드려요.</div>
          ...
          <button onClick={async () => { setMode("company"); await saveStorage("eval-mode", "company"); }} style={{ width: "100%", background: "#fff", border: `2px solid ${C.purple}`, borderRadius: 16, padding: "20px 18px", marginBottom: 24, display: "flex", alignItems: "flex-start", gap: 14, cursor: "pointer", textAlign: "left" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: C.slate, lineHeight: 1.6 }}>기존 엑셀 양식을 업로드하면 AI가 내용을 자동으로 채워드려요.</div>
              <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", marginBottom: 12 }}>
            ...
            {companyTemplate ? (
              <div>
                ...
                <button onClick={fillTemplateWithAI} disabled={templateLoading} style={{ width: "100%", padding: "14px", background: templateLoading ? "rgba(139,92,246,0.3)" : `linear-gradient(135deg, #7c3aed, ${C.purple})`, border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: templateLoading ? "not-allowed" : "pointer" }}>
                  {templateLoading ? "AI가 양식을 채우는 중..." : "🤖 AI로 양식 자동채우기 + 다운로드"}
                </button>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 12, lineHeight: 1.6 }}>회사 위험성평가 엑셀 양식을 업로드하면 AI가 자동으로 내용을 채워드려요.</div>
        ...
        {tab === "assessment" && (
          <div style={{ maxWidth: 560, margin: "0 auto", padding: "10px 14px 28px" }}>
            {completedSteps.length > 0 && (
              <button onClick={downloadAllExcel} style={{ width: "100%", padding: "13px", marginBottom: 12, background: "linear-gradient(135deg, #16a34a, #22c55e)", border: "none", borderRadius: 13, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "16px 14px 32px" }}>
          {loading ? (
            ...
            <div>
              ...
              {allDone && (
                <button onClick={downloadAllExcel} style={{ width: "100%", padding: "14px", marginBottom: 12, background: "linear-gradient(135deg, #16a34a, #22c55e)", border: "none", borderRadius: 13, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
