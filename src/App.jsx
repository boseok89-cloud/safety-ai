  ...
  const Header = ({ title, onBack }) => (
    <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, padding: "14px 16px", position: "sticky", top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 560, margin: "0 auto", display: "flex", alignItems: "center", gap: 10 }}>
        {onBack && (
          <button onClick={onBack} style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 8, padding: "6px 11px", color: "#fff", fontSize: 13, cursor: "pointer" }}>← 뒤로</button>
        )}
  ...
  const BaseInfoBanner = () => (
    baseConfirmed && baseInfo.company ? (
      <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 11, padding: "10px 14px", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => setScreen("step-form")} style={{ background: "none", border: "1px solid rgba(34,197,94,0.4)", borderRadius: 7, padding: "4px 10px", color: "#166534", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>수정</button>
      </div>
    ) : null
  );
        ...
        <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, padding: "20px 16px 16px" }}>
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              ...
              <button onClick={() => setShowProfileModal(true)} style={{ background: baseConfirmed ? "rgba(34,197,94,0.25)" : "rgba(255,255,255,0.12)", border: `1px solid ${baseConfirmed ? "rgba(34,197,94,0.5)" : "rgba(255,255,255,0.2)"}`, borderRadius: 10, padding: "7px 12px", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                {baseConfirmed ? "✅ 프로필" : "🏢 프로필"}
              </button>
            </div>
            {baseConfirmed ? (
              <div style={{ background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 10, padding: "10px 14px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 18 }}>🏢</span>
                <div style={{ flex: 1 }}>
                  <div style={{ color: "#fff", fontSize: 13, fontWeight: 700 }}>{baseInfo.company}</div>
                  <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 11, marginTop: 1 }}>{baseInfo.industry} · {baseInfo.workers} · {baseInfo.manager}</div>
                </div>
                <div style={{ color: "#4ade80", fontSize: 11, fontWeight: 700 }}>전 단계 자동적용</div>
              </div>
            ) : (
              <div style={{ background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 12, color: "#fbbf24" }}>
                💡 STEP 1 사전준비에서 사업장 정보를 입력하면 모든 단계에 자동 적용돼요
              </div>
            )}
            <div style={{ display: "flex", gap: 4 }}>
              {STEPS.map((s, i) => (
                <div key={s.id} style={{ flex: 1, height: 5, borderRadius: 3, background: completedSteps.includes(i + 1) ? C.green : "rgba(255,255,255,0.18)", transition: "all 0.4s" }} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 14px 0" }}>
          <div style={{ display: "flex", background: "#e2e8f0", borderRadius: 11, padding: 3, gap: 3 }}>
            {[{ k: "assessment", l: "📋 위험성평가" }, { k: "education", l: "🎓 교육자료" }, { k: "history", l: "📜 이력" }].map(t => (
              <button key={t.k} onClick={() => setTab(t.k)} style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: "none", background: tab === t.k ? "#fff" : "transparent", color: tab === t.k ? C.navy : C.slate, fontSize: 12, fontWeight: 700, cursor: "pointer", boxShadow: tab === t.k ? "0 2px 6px rgba(0,0,0,0.08)" : "none" }}>{t.l}</button>
            ))}
          </div>
        </div>
        ...
        {tab === "assessment" && (
          <div style={{ maxWidth: 560, margin: "0 auto", padding: "12px 14px 28px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: "14px", boxShadow: "0 1px 5px rgba(0,0,0,0.05)", marginBottom: 12 }}>
              ...
              {companyTemplate ? (
                <div>
                  <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 9, padding: "10px 13px", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                  ...
                  <button onClick={fillTemplateWithAI} disabled={templateLoading} style={{ width: "100%", padding: "11px", background: templateLoading ? "rgba(139,92,246,0.3)" : "linear-gradient(135deg, #7c3aed, #8b5cf6)", border: "none", borderRadius: 10, color: "#fff", fontSize: 13, fontWeight: 700, cursor: templateLoading ? "not-allowed" : "pointer" }}>
                    {templateLoading ? "⏳ AI가 양식을 채우는 중..." : "🤖 AI로 양식 자동채우기 + 다운로드"}
                  </button>
                </div>
              ) : (
                <div>
                  <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 10, lineHeight: 1.5 }}>회사 위험성평가 엑셀 양식을 업로드하면 AI가 자동으로 내용을 채워드려요</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {STEPS.map((s, i) => {
                ...
                return (
                  <button key={s.id} onClick={() => { setActiveStep(s); setStepData({}); setResult(""); setScreen("step-form"); }} style={{ background: "#fff", border: `2px solid ${done ? C.green : "#e2e8f0"}`, borderRadius: 13, padding: "13px 15px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left", boxShadow: "0 1px 5px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ marginTop: 12, padding: "11px 14px", background: "rgba(14,165,233,0.07)", border: "1px solid rgba(14,165,233,0.18)", borderRadius: 10, fontSize: 12, color: "#0369a1", lineHeight: 1.7 }}>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }} onClick={() => setShowScenario(false)}>
            <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", padding: "20px 16px 36px", width: "100%", maxWidth: 560 }} onClick={e => e.stopPropagation()}>
              ...
              {Object.entries(INDUSTRY_SCENARIOS).map(([name, sc]) => (
                <button key={name} onClick={() => { applyScenario(name); setBaseInfo(p => ({ ...p, industry: p.industry || name })); }} style={{ width: "100%", background: "#f8fafc", border: "2px solid #e2e8f0", borderRadius: 11, padding: "12px 14px", textAlign: "left", cursor: "pointer", marginBottom: 8 }}>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 14px 32px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", marginBottom: 12 }}>
            ...
            {activeStep.hasScenario && (
              <button onClick={() => setShowScenario(true)} style={{ width: "100%", padding: "9px", marginBottom: 14, background: "rgba(245,158,11,0.08)", border: "1.5px solid rgba(245,158,11,0.3)", borderRadius: 9, color: C.amber, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🏭 업종별 시나리오로 자동완성</button>
            )}
          ...
          <button onClick={async () => { if (isStep1) setBaseConfirmed(true); setScreen("step-result"); await callAI(activeStep.prompt); setCompletedSteps(prev => prev.includes(activeStep.id) ? prev : [...prev, activeStep.id]); }} style={{ width: "100%", padding: "14px", background: `linear-gradient(135deg, ${stepColor}, ${stepColor}cc)`, border: "none", borderRadius: 13, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 14px ${stepColor}44` }}>
            🤖 AI 문서 자동 작성
          </button>
        </div>
        {showScenario && (
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 14px 32px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", marginBottom: 12 }}>
            {[
              ...
              <div key={f.key} style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 700, color: "#374151", display: "block", marginBottom: 5 }}>{f.label}</label>
                <input value={stepData[f.key] || ""} onChange={e => setStepData(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder} style={{ width: "100%", padding: "10px 13px", borderRadius: 9, border: "1.5px solid #e2e8f0", fontSize: 14, color: C.navy, outline: "none", background: "#f8fafc", boxSizing: "border-box" }} />
              </div>
            ))}
          </div>
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #f0f4f8 0%, #e8eef5 100%)", fontFamily: "'Noto Sans KR', sans-serif" }}>
        <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, padding: "14px 16px", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 20px rgba(0,0,0,0.2)" }}>
          <div style={{ maxWidth: 560, margin: "0 auto", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1, color: "#fff", fontSize: 15, fontWeight: 700 }}>{activeStep.icon || "🎓"} {activeStep.title} 결과</div>
            {!loading && result && (
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button onClick={downloadExcel} style={{ background: "rgba(34,197,94,0.3)", border: "1px solid rgba(34,197,94,0.5)", borderRadius: 8, padding: "6px 10px", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>📥 엑셀</button>
              </div>
            )}
          </div>
        </div>
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "16px 14px 32px" }}>
          {loading ? (
            ...
            <div>
              ...
              <button onClick={downloadExcel} style={{ width: "100%", padding: "13px", marginBottom: 14, background: "linear-gradient(135deg, #16a34a, #22c55e)", border: "none", borderRadius: 13, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px rgba(34,197,94,0.4)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 4px 24px rgba(0,0,0,0.06)", overflow: "hidden", marginBottom: 12 }}>
                <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {nextStep && (
                <button onClick={() => { setActiveStep(nextStep); setStepData({}); setResult(""); setScreen("step-form"); }} style={{ width: "100%", padding: "14px", marginBottom: 8, background: `linear-gradient(135deg, ${nextStep.color}, ${nextStep.color}cc)`, border: "none", borderRadius: 13, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 14px ${nextStep.color}44` }}>
              ...
              <div style={{ marginTop: 12, padding: "12px 14px", background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, fontSize: 12, color: "#92400e", lineHeight: 1.6 }}>
                ⚠️ AI 초안입니다. 안전관리자가 현장 상황에 맞게 반드시 검토·수정 후 사용하세요.
              </div>
            </div>
          )}
